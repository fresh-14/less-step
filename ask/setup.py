...
sys.path.append('/home/aleks/projects/stepik/www/web/ask')
...
class TestInitData(unittest.TestCase):
    def test_import(self):
        ...
        for i in range(30):
            question = Question.objects.create(title='question ' + str(i),
                                               text='text ' + str(i),
                                               author=user,
                                               rating=max_rating + i)
        time.sleep(2)
...
suite = unittest.TestLoader().loadTestsFromTestCase(TestInitData)
